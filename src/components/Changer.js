...
export default class Chenger {
    constructor(config = {enableLKey: false}) {
        this.config = config;
        ...
        this.romanTable['ヴぁ'] = this.romanTable['ゔぁ']; this.romanTable['ヴぃ'] = this.romanTable['ゔぃ'];
        this.romanTable['ヴ'] = this.romanTable['ゔ']; this.romanTable['ヴぇ'] = this.romanTable['ゔぇ'];
        ...
        for (const [hiraganas, cons] of [
            ...
            for (let i = 0, _i = hiraganas.length; i < _i; i++) {
                ...
                this.romanTable[hiragana] = this.consonant[hiragana].split(',').map(c => c + 'aiueo'[i]);
            }
        }
    ...
    getRoman(furigana, targetPos) {
        ...
        if (isSmallChar(nextChar) && this.romanTable[nowChar + nextChar]) result = [this.romanTable[nowChar + nextChar].concat(), 2]; // 「じゃ」 などthis.romanTableに登録されている場合
            ...
            for (const cons of this.consonant[nowChar].split(',')) result.push(cons + smallChar[0], cons + smallChar[1]);
            ...
            // 必要最低限のnで返す
            result = ['nn'];
            if (nextChar !== '' && (this.consonant[nextChar] === undefined || !['n', '', 'y'].includes(this.consonant[nextChar])))
            ...
            if (nextChar !== '' && this.consonant[nextChar] !== undefined) {
                const [_res, count] = new Chenger({enableLKey: false}).getRoman(furigana, targetPos + 1);
                const _add = str => _res.map(add(str));
                result = [[..._res.map(item => item[0] + item), ..._add('xtu'), ..._add('xtsu')], count + 1];
            }
        } else result = [this.romanTable[nowChar].concat(), 1]; // 普通のとき

        if (this.config.enableLKey) {
            const target = result[0].filter(s => s.indexOf('x') !== -1);
            result[0] = result[0].concat(target.reduce((res, item) => {
        ...
        if (furigana[targetPos - 1] === 'ん' && !['n', '', 'y'].includes(this.consonant[nowChar])) {
            // ここはnを足す処理
